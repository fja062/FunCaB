...
# climate space map
x <- comp2 %>% 
  #mutate(precipLevel = factor(precip, levels = c(600, 1200, 2000, 2700), labels = c("dry", " ", "  ", "wet")), tempLevel = factor(temp, levels = c(6.5, 8.5, 10.5), labels = c("alpine", "sub-alpine", "boreal"))) %>% 
  ggplot(aes(x = precip7010, y = temp7010, fill = factor(precipLevel), shape = factor(tempLevel))) +
  geom_point(size = 5) +
  ...
  scale_y_reverse(lim = c(12.5, 5)) +
  xlim(c(300, 3400)) +
  labs(x = "Annual precipitation (mm)", y = "Mean summer temperature (°C)") +
  axis.dimLarge +
  theme(legend.position = "none")
  ggsave(x, filename = "~/OneDrive - University of Bergen/Research/FunCaB/paper 1/figures/climSpace.png", dpi = 300, height = 4, width = 6.5, bg = "transparent")
